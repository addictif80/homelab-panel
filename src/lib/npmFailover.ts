 ...
 * failover — it only probes both targets periodically to keep a status ("primaire actif" /
 * "basculé sur le secours" / "erreur") for display, independent of nginx's own real-time behavior.
 */
...
function buildSnippet(backupScheme: string, backupHost: string, backupPort: number): string {
  const locationName = "@homelab_failover_backup";
  return [
    MARKER_START,
    "proxy_intercept_errors on;",
    `error_page 502 503 504 = ${locationName};`,
    `location ${locationName} {`,
    "  internal;",
    `  proxy_pass ${backupScheme}://${backupHost}:${backupPort};`,
...
export async function applyFailover(
  proxyHostId: number,
  backup: { scheme: "http" | "https"; host: string; port: number }
): Promise<void> {
  const host = await getProxyHost(proxyHostId);
  const snippet = buildSnippet(backup.scheme, backup.host, backup.port);
  await pushAdvancedConfig(host, snippet);

  getDb()
    .prepare(
      `INSERT INTO proxy_failovers (proxy_host_id, backup_scheme, backup_host, backup_port, enabled)
       VALUES (?, ?, ?, ?, 1)
    ...
    .run(proxyHostId, backup.scheme, backup.host, backup.port);
}
  ...
  await Promise.all(
    configs.map(async (config) => {
      try {
        ...
        const status = primaryUp ? "primary" : (await probe(config.backupScheme, config.backupHost, config.backupPort)) ? "failover" : "error";
        getDb()
