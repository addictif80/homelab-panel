 ...
 */
function buildLsyncdConfig(entries: { sourcePath: string; targetPath: string; targetUser: string; targetAddress: string; targetPort: number; keyPath: string }[]): string {
...
async function reconcileLsyncdOnHost(hostId: number): Promise<void> {
  ...
  const built = await Promise.all(
    entries.map(async (r) => {
      ...
      return { sourcePath: r.sourcePath, targetPath: r.targetPath, targetUser: dest.user, targetAddress: dest.address, targetPort: dest.port, keyPath };
    })
  );
