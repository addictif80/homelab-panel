...
import { sendInput, type BrowserInput } from "@/lib/browserSession";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const input = (await req.json().catch(() => null)) as BrowserInput | null;
  if (!input || typeof input.type !== "string" || !VALID_TYPES.has(input.type)) {
    return NextResponse.json({ error: "Entrée invalide." }, { status: 400 });
  }

  const ok = await sendInput(id, username, input);
