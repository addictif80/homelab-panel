...
export async function GET() {
  return NextResponse.json({ links: listServiceLinks() });
}
...
export async function POST(req: NextRequest) {
  ...
  const faviconDataUrl = await fetchFavicon(url);
  const link = createServiceLink({ name, url, faviconDataUrl, showPublic });
